        ...
        for (int i = 0; i < (int) pop.size(); i++) {
            ...
            if ((int) pop.size() <= params->nbElite) // Elite individuals cannot be smaller than population size
                ...
                pop[ranking[i].second]->biasedFitness =
                        fitRank + (1.0 - (double) params->nbElite / (double) pop.size()) * divRank;
        }
    }
}
...
void Population::removeWorstBiasedFitness(SubPopulation &pop) {
    ...
    for (int i = 1; i < (int) pop.size(); i++) {
        bool isClone = (pop[i]->averageBrokenPairsDistanceClosest(1) <
                        MY_EPSILON); // A distance equal to 0 indicates that a clone exists
    ...
    for (Individual *myIndividual2 : pop)
        myIndividual2->removeProximity(
                worstIndividual); // Cleaning its distances from the other individuals in the population
...
void Population::managePenalties() {
    ...
    double fractionFeasibleLoad = (double) std::count(listFeasibilityLoad.begin(), listFeasibilityLoad.end(), true) /
                                  (double) listFeasibilityLoad.size();
    if (fractionFeasibleLoad < params->targetFeasible - 0.05 && params->penaltyCapacity < 1000)
        params->penaltyCapacity = std::min<double>(params->penaltyCapacity * 1.2, 1000.);
    else if (fractionFeasibleLoad > params->targetFeasible + 0.05 && params->penaltyCapacity > 0.1)
        params->penaltyCapacity = std::max<double>(params->penaltyCapacity * 0.85, 0.1);

    // Setting some bounds [0.1,1000] to the penalty values for safety
    double fractionFeasibleDuration =
            (double) std::count(listFeasibilityDuration.begin(), listFeasibilityDuration.end(), true) /
            (double) listFeasibilityDuration.size();
    if (fractionFeasibleDuration < params->targetFeasible - 0.05 && params->penaltyDuration < 1000)
        params->penaltyDuration = std::min<double>(params->penaltyDuration * 1.2, 1000.);
    else if (fractionFeasibleDuration > params->targetFeasible + 0.05 && params->penaltyDuration > 0.1)
        params->penaltyDuration = std::max<double>(params->penaltyDuration * 0.85, 0.1);
    ...
    for (int i = 0; i < (int) infeasibleSubpopulation.size(); i++)
        infeasibleSubpopulation[i]->myCostSol.penalizedCost = infeasibleSubpopulation[i]->myCostSol.distance
                                                              ...
                                                              + params->penaltyDuration *
                                                                infeasibleSubpopulation[i]->myCostSol.durationExcess;
...
void Population::printState(int nbIter, int nbIterNoImprovement) {
    ...
    std::printf(" | Feas %.2f %.2f", (double) std::count(listFeasibilityLoad.begin(), listFeasibilityLoad.end(), true) /
                                     (double) listFeasibilityLoad.size(),
                (double) std::count(listFeasibilityDuration.begin(), listFeasibilityDuration.end(), true) /
                (double) listFeasibilityDuration.size());
...
double Population::getDiversity(const SubPopulation &pop) {
    double average = 0.;
    int size = std::min<int>(params->mu,
                             pop.size()); // Only monitoring the "mu" better solutions to avoid too much noise in the measurements
...
Population::Population(Params *params, Split *split, LocalSearch *localSearch) : params(params), split(split),
