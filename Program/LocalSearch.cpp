    ...
    for (loopID = 0; !searchCompleted; loopID++) {
        ...
        for (int rU = 0; rU < params->nbVehicles; rU++) {
            ...
            for (int rV = 0; rV < params->nbVehicles; rV++) {
                ...
                if (routeU->nbCustomers > 0 && routeV->nbCustomers > 0 && routeU->cour < routeV->cour && (loopID == 0 ||
                                                                                                          std::max<int>(
                                                                                                                  routeU->whenLastModified,
...
bool LocalSearch::move1() {
    double costSuppU = params->timeCost[nodeUPrevIndex][nodeXIndex] - params->timeCost[nodeUPrevIndex][nodeUIndex] -
                       params->timeCost[nodeUIndex][nodeXIndex];
    double costSuppV = params->timeCost[nodeVIndex][nodeUIndex] + params->timeCost[nodeUIndex][nodeYIndex] -
...
bool LocalSearch::move2() {
    double costSuppU = params->timeCost[nodeUPrevIndex][nodeXNextIndex] - params->timeCost[nodeUPrevIndex][nodeUIndex] -
                       params->timeCost[nodeXIndex][nodeXNextIndex];
    double costSuppV = params->timeCost[nodeVIndex][nodeUIndex] + params->timeCost[nodeXIndex][nodeYIndex] -
                       params->timeCost[nodeVIndex][nodeYIndex];

    if (routeU != routeV) {
        costSuppU += penaltyExcessDuration(
                             routeU->duration + costSuppU - params->timeCost[nodeUIndex][nodeXIndex] - serviceU -
                             serviceX)
                     + penaltyExcessLoad(routeU->load - loadU - loadX)
                     - routeU->penalty;

        costSuppV += penaltyExcessDuration(
                             routeV->duration + costSuppV + params->timeCost[nodeUIndex][nodeXIndex] + serviceU +
                             serviceX)
...
bool LocalSearch::move3() {
    double costSuppU = params->timeCost[nodeUPrevIndex][nodeXNextIndex] - params->timeCost[nodeUPrevIndex][nodeUIndex] -
                       params->timeCost[nodeUIndex][nodeXIndex] - params->timeCost[nodeXIndex][nodeXNextIndex];
    double costSuppV = params->timeCost[nodeVIndex][nodeXIndex] + params->timeCost[nodeXIndex][nodeUIndex] +
                       params->timeCost[nodeUIndex][nodeYIndex] - params->timeCost[nodeVIndex][nodeYIndex];
...
bool LocalSearch::move4() {
    double costSuppU = params->timeCost[nodeUPrevIndex][nodeVIndex] + params->timeCost[nodeVIndex][nodeXIndex] -
                       params->timeCost[nodeUPrevIndex][nodeUIndex] - params->timeCost[nodeUIndex][nodeXIndex];
    double costSuppV = params->timeCost[nodeVPrevIndex][nodeUIndex] + params->timeCost[nodeUIndex][nodeYIndex] -
                       params->timeCost[nodeVPrevIndex][nodeVIndex] - params->timeCost[nodeVIndex][nodeYIndex];
...
bool LocalSearch::move5() {
    double costSuppU = params->timeCost[nodeUPrevIndex][nodeVIndex] + params->timeCost[nodeVIndex][nodeXNextIndex] -
                       params->timeCost[nodeUPrevIndex][nodeUIndex] - params->timeCost[nodeXIndex][nodeXNextIndex];
    double costSuppV = params->timeCost[nodeVPrevIndex][nodeUIndex] + params->timeCost[nodeXIndex][nodeYIndex] -
                       params->timeCost[nodeVPrevIndex][nodeVIndex] - params->timeCost[nodeVIndex][nodeYIndex];

    if (routeU != routeV) {
        costSuppU += penaltyExcessDuration(
                             routeU->duration + costSuppU - params->timeCost[nodeUIndex][nodeXIndex] + serviceV -
                             serviceU - serviceX)
                     + penaltyExcessLoad(routeU->load + loadV - loadU - loadX)
                     - routeU->penalty;

        costSuppV += penaltyExcessDuration(
                             routeV->duration + costSuppV + params->timeCost[nodeUIndex][nodeXIndex] - serviceV +
                             serviceU + serviceX)
...
bool LocalSearch::move6() {
    double costSuppU = params->timeCost[nodeUPrevIndex][nodeVIndex] + params->timeCost[nodeYIndex][nodeXNextIndex] -
                       params->timeCost[nodeUPrevIndex][nodeUIndex] - params->timeCost[nodeXIndex][nodeXNextIndex];
    double costSuppV = params->timeCost[nodeVPrevIndex][nodeUIndex] + params->timeCost[nodeXIndex][nodeYNextIndex] -
                       params->timeCost[nodeVPrevIndex][nodeVIndex] - params->timeCost[nodeYIndex][nodeYNextIndex];

    if (routeU != routeV) {
        costSuppU += penaltyExcessDuration(routeU->duration + costSuppU - params->timeCost[nodeUIndex][nodeXIndex] +
                                           params->timeCost[nodeVIndex][nodeYIndex] + serviceV + serviceY - serviceU -
                                           serviceX)
                     + penaltyExcessLoad(routeU->load + loadV + loadY - loadU - loadX)
                     - routeU->penalty;

        costSuppV += penaltyExcessDuration(routeV->duration + costSuppV + params->timeCost[nodeUIndex][nodeXIndex] -
                                           params->timeCost[nodeVIndex][nodeYIndex] - serviceV - serviceY + serviceU +
                                           serviceX)
...
bool LocalSearch::move7() {
    ...
    double cost = params->timeCost[nodeUIndex][nodeVIndex] + params->timeCost[nodeXIndex][nodeYIndex] -
...
bool LocalSearch::move8() {
    double cost = params->timeCost[nodeUIndex][nodeVIndex] + params->timeCost[nodeXIndex][nodeYIndex] -
                  ...
                  + penaltyExcessDuration(
                          nodeU->cumulatedTime + nodeV->cumulatedTime + nodeV->cumulatedReversalDistance +
                          params->timeCost[nodeUIndex][nodeVIndex])
                  + penaltyExcessDuration(
                          routeU->duration - nodeU->cumulatedTime - params->timeCost[nodeUIndex][nodeXIndex] +
                          routeU->reversalDistance - nodeX->cumulatedReversalDistance + routeV->duration -
                          nodeV->cumulatedTime - params->timeCost[nodeVIndex][nodeYIndex] +
...
bool LocalSearch::move9() {
    double cost = params->timeCost[nodeUIndex][nodeYIndex] + params->timeCost[nodeVIndex][nodeXIndex] -
                  params->timeCost[nodeUIndex][nodeXIndex] - params->timeCost[nodeVIndex][nodeYIndex]
                  + penaltyExcessDuration(nodeU->cumulatedTime + routeV->duration - nodeV->cumulatedTime -
                                          params->timeCost[nodeVIndex][nodeYIndex] +
                                          params->timeCost[nodeUIndex][nodeYIndex])
                  + penaltyExcessDuration(
                          routeU->duration - nodeU->cumulatedTime - params->timeCost[nodeUIndex][nodeXIndex] +
                          nodeV->cumulatedTime + params->timeCost[nodeVIndex][nodeXIndex])
...
bool LocalSearch::swapStar() {
    ...
    for (nodeU = routeU->depot->next; !nodeU->isDepot; nodeU = nodeU->next) {
        for (nodeV = routeV->depot->next; !nodeV->isDepot; nodeV = nodeV->next) {
            double deltaPenRouteU = penaltyExcessLoad(
                    routeU->load + params->cli[nodeV->cour].demand - params->cli[nodeU->cour].demand) - routeU->penalty;
            double deltaPenRouteV = penaltyExcessLoad(
                    routeV->load + params->cli[nodeU->cour].demand - params->cli[nodeV->cour].demand) - routeV->penalty;
            ...
            if (deltaPenRouteU + nodeU->deltaRemoval + deltaPenRouteV + nodeV->deltaRemoval <= 0) {
                ...
                mySwapStar.moveCost =
                        deltaPenRouteU + nodeU->deltaRemoval + extraU + deltaPenRouteV + nodeV->deltaRemoval + extraV
                        + penaltyExcessDuration(routeU->duration + nodeU->deltaRemoval + extraU +
                                                params->cli[nodeV->cour].serviceDuration -
                                                params->cli[nodeU->cour].serviceDuration)
    ...
    for (nodeU = routeU->depot->next; !nodeU->isDepot; nodeU = nodeU->next) {
        ...
        double deltaDistRouteU = params->timeCost[nodeU->prev->cour][nodeU->next->cour] -
                                 params->timeCost[nodeU->prev->cour][nodeU->cour] -
                                 params->timeCost[nodeU->cour][nodeU->next->cour];
        ...
        mySwapStar.moveCost = deltaDistRouteU + deltaDistRouteV
                              ...
                              + penaltyExcessDuration(
                                      routeU->duration + deltaDistRouteU - params->cli[nodeU->cour].serviceDuration)
                              + penaltyExcessDuration(
                                      routeV->duration + deltaDistRouteV + params->cli[nodeU->cour].serviceDuration);
    ...
    for (nodeV = routeV->depot->next; !nodeV->isDepot; nodeV = nodeV->next) {
        ...
        double deltaDistRouteV = params->timeCost[nodeV->prev->cour][nodeV->next->cour] -
                                 params->timeCost[nodeV->prev->cour][nodeV->cour] -
                                 params->timeCost[nodeV->cour][nodeV->next->cour];
        mySwapStar.moveCost = deltaDistRouteU + deltaDistRouteV
                              ...
                              + penaltyExcessDuration(
                                      routeU->duration + deltaDistRouteU + params->cli[nodeV->cour].serviceDuration)
                              + penaltyExcessDuration(
                                      routeV->duration + deltaDistRouteV - params->cli[nodeV->cour].serviceDuration);
...
double LocalSearch::getCheapestInsertSimultRemoval(Node *U, Node *V, Node *&bestPosition) {
    ...
    double deltaCost = params->timeCost[V->prev->cour][U->cour] + params->timeCost[U->cour][V->next->cour] -
                       params->timeCost[V->prev->cour][V->next->cour];
...
void LocalSearch::preprocessInsertions(Route *R1, Route *R2) {
    for (Node *U = R1->depot->next; !U->isDepot; U = U->next) {
        ...
        U->deltaRemoval = params->timeCost[U->prev->cour][U->next->cour] - params->timeCost[U->prev->cour][U->cour] -
        ...
        if (R2->whenLastModified > bestInsertClient[R2->cour][U->cour].whenLastCalculated) {
            ...
            bestInsertClient[R2->cour][U->cour].bestCost[0] =
                    params->timeCost[0][U->cour] + params->timeCost[U->cour][R2->depot->next->cour] -
                    params->timeCost[0][R2->depot->next->cour];
            bestInsertClient[R2->cour][U->cour].bestLocation[0] = R2->depot;
            for (Node *V = R2->depot->next; !V->isDepot; V = V->next) {
                double deltaCost = params->timeCost[V->cour][U->cour] + params->timeCost[U->cour][V->next->cour] -
...
void LocalSearch::updateRouteData(Route *myRoute) {
    ...
    while (!mynode->isDepot || firstIt) {
        ...
        myReversalDistance +=
                params->timeCost[mynode->cour][mynode->prev->cour] - params->timeCost[mynode->prev->cour][mynode->cour];
    ...
    if (myRoute->nbCustomers == 0) {
        ...
        myRoute->polarAngleBarycenter = atan2(cumulatedY / (double) myRoute->nbCustomers - params->cli[0].coordY,
                                              cumulatedX / (double) myRoute->nbCustomers - params->cli[0].coordX);
...
void LocalSearch::exportIndividual(Individual *indiv) {
    ...
    std::sort(routePolarAngles.begin(),
              routePolarAngles.end()); // empty routes have a polar angle of 1.e30, and therefore will always appear at the end
